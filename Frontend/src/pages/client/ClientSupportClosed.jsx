    ...
    const statusBadge = (status) => {
        ...
        return (
            <span style={{
                ...
                background: isRejected ? '#1e1b4b' : '#f1f5f9',
                color: isRejected ? '#fff' : '#334155',
                border: isRejected ? 'none' : '1px solid #cbd5e1',
            }}>
                {isRejected ? Icons.XCircle : Icons.Archive}
                {status}
            </span>
        );
    };
    ...
    return (
        <div>
            ...
            <div className="page-header">
                <div>
                    <h2 style={{ color: '#1e1b4b' }}>Closed Support</h2>
                    <p style={{ color: '#475569' }}>Resolved or completed support tickets.</p>
                </div>
                {requests.length > 0 && (
                    <div style={{
                        ...
                        {Icons.Archive}
                        {requests.length} Closed
                    </div>
                )}
            </div>

            {/* Closed Support Requests Table */}
            <div className="glass-card table-section">
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
                    <div style={{
                        width: '36px', height: '36px', background: '#f1f5f9',
                        borderRadius: '8px', display: 'flex', alignItems: 'center',
                        justifyContent: 'center', color: '#1e1b4b',
                        border: '1px solid #e2e8f0'
                    }}>
                        {Icons.Archive}
                    </div>
                    <div>
                        <h3 style={{ margin: 0, color: '#1e1b4b', fontSize: '1.05rem' }}>Closed Tickets</h3>
                        <span style={{ color: '#64748b', fontSize: '0.8rem' }}>Resolved &amp; rejected requests</span>
                    </div>
                </div>
                ...
                <div className="data-table-container">
                    <table className="data-table">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="7" style={{ textAlign: 'center', padding: '36px', color: '#64748b' }}>Loading closed tickets...</td></tr>
                            ) : requests.length === 0 ? (
                                <tr>
                                    <td colSpan="7" style={{ textAlign: 'center', padding: '40px' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', color: '#94a3b8' }}>
                                            ...
                                            <span>No closed support requests yet.</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={req.id}>
                                        ...
                                        <td>
                                            <span style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#334155' }}>
                                                {Icons.User}
                                                {req.assignedManager || <span style={{ color: '#94a3b8', fontStyle: 'italic' }}>N/A</span>}
                                            </span>
                                        </td>
                                        <td>
...
export default ClientSupportClosed;
